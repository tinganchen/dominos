...
def main():
    ...
    checkpoint_t = torch.load(args.teacher_dir + args.teacher_file, map_location = device)
    state_dict = checkpoint_t['state_dict_s']
    ...
    # pruned model
    model = import_module('utils.preprocess').__dict__[f'{args.arch}'](args, checkpoint['state_dict_s'], args.t)
...
def compressionInfo(flops, params, org_gflops = 0.31469, org_params = 15):
    GFLOPs = flops / 10 ** 9
    params_num = params
