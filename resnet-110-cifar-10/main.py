...
def compressionInfo(epoch, flops, params, test_prec1, test_prec5, org_gflops = 0.25604, org_params = 1.7401079956118397):
    GFLOPs = flops / 10 ** 9
    params_num = params
...
def train(args, loader_train, models, optimizers, epoch):
    ...
    for i, (inputs, targets) in enumerate(loader_train, 1):
        ...
        if args.arch == 'resnet':
            ...
            for layer in [model_s.layer1, model_s.layer2, model_s.layer3]:
                for block in layer:                  
            ...
            for j in range(int(len(mask)/2)):
                ratios.append((sum(mask[j * 2] > 0.0) + sum(mask[j * 2 + 1] > 0.0)) / (mask[j * 2].shape[0] + mask[j * 2 + 1].shape[0]))
