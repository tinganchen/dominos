...
def main():
    ...
    model_t = torchvision.models.resnet50(pretrained = True).to(device)
    ...
    param_s = [param for name, param in model_s.named_parameters() if 'alpha' not in name]
    param_m = [param for name, param in model_s.named_parameters() if 'alpha' in name and 'mask.' not in name]   
    param_b = [param for name, param in model_s.named_parameters() if 'mask.' in name]   
    ...
    optimizer_b = FISTA(param_b, lr = args.lr, gamma = args.sparse_lambda2)
    ...
    scheduler_b = StepLR(optimizer_b, step_size = args.lr_decay_step, gamma = 0.1)
    ...
    if resume:
        ...
        scheduler_b.load_state_dict(ckpt['scheduler_b'])
    ...
    optimizers = [optimizer_s, optimizer_m, optimizer_b]
    schedulers = [scheduler_s, scheduler_m, scheduler_b]
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        state = {
            ...
            'scheduler_b': scheduler_b.state_dict(),
        ...
        model_p = import_module('utils.preprocess').__dict__[f'{args.arch}'](args, model_s.state_dict(), args.t) # args.thres
        flops, params = get_model_complexity_info(model_p.to(device), (3, 224, 224), as_strings = False, print_per_layer_stat = False)
...
def compressionInfo(epoch, flops, params, test_prec1, test_prec5, org_gflops = 4.121924096, org_params = 25.579704):
    GFLOPs = flops / 10 ** 9
    params_num = params
...
def train(args, loader_train, models, optimizers, epoch):
    ...
    losses_block = utils.AverageMeter()
    ...
    for i, (inputs, targets) in enumerate(loader_train, 1):
        ...
        if args.arch == 'resnet':
            ...
            mask = []
            mask_block = []
            #mask.append(model_s.mask.alpha.view(-1))
            for layer in [model_s.layer1, model_s.layer2, model_s.layer3, model_s.layer4]:
                for block in layer:                  
                    mask.append(block.mask1.alpha.view(-1))
                    mask.append(block.mask2.alpha.view(-1))
                    mask.append(block.mask3.alpha.view(-1))
                    mask_block.append(block.mask.alpha.view(-1))
    
            mask_block = torch.cat(mask_block)

            ratios = [] # ratios of non-zero elements
            for j in range(int(len(mask)/3)):
                ratios.append((sum(mask[j * 3] > 0.0) + sum(mask[j * 3 + 1] > 0.0) + sum(mask[j * 3 + 2] > 0.0)) / (mask[j * 3].shape[0] + mask[j * 3 + 1].shape[0] + mask[j * 3 + 2].shape[0]))
            ratios = torch.FloatTensor(ratios).to(device)
            
            error_block = 1 - torch.sum(mask_block * ratios) / torch.norm(mask_block, 2) / torch.norm(ratios, 2)
            error_block.backward(retain_graph = True)
            
            
            mask = torch.cat(mask)
            error_sparse = args.sparse_lambda * (torch.norm(mask, 1) / len(mask)) + args.sparse_lambda2 * torch.norm(mask_block, 1) / len(mask_block)
            ...
            losses_block.update(error_block.item(), inputs.size(0))
            writer_train.add_scalar('Block_pruning_loss', error_sparse.item(), num_iters)
            ...
            if args.t > 0:
                ...
                error_info_cascade.backward(retain_graph = True)
                
                losses_cascade.update(error_info_cascade.item(), inputs.size(0))
                writer_train.add_scalar('Cascade_fit_loss', error_info_cascade.item(), num_iters)
        ...
        if i % args.print_freq == 0:
            if args.t > 0:
                print_logger.info(
                    ...
                    'Block_loss: {block_loss.val:.4f} ({block_loss.avg:.4f})\n'
