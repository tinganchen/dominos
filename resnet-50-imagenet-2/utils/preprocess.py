...
from model.resnet import pruned_resnet_50_sparse, resnet_50_sparse
import torch
...
def resnet(args, state_dict, t, thres = None): # thres
    ...
    has_mask = [1 if m > 0.0 else 0 for m in mask_block]
    ...
    pruned_model = pruned_resnet_50_sparse(has_mask = has_mask).to(device)
    ...
    if not args.random:
        for key in list(new_state_dict.keys()):
            if key not in list(pruned_model.state_dict().keys()):
